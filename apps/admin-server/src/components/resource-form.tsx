...
export default function ResourceForm({ onFormSubmit }: Props) {
	...
	const loadedStatuses = (statuses || []) as {
		id: number;
		name: string;
	}[];
	...
	const budgetFallback = (existingData: any, key = "") => {
	...
	useEffect(() => {
		if (existingData) {
			...
			const resetValues: { tags?: number[]; statuses?: number[] } = {};
	...
	const handleLocationSelect = useCallback(
		(location: string) => {
			if (location !== "") {
				const formatted = location.split(",");
				form.setValue(
					"location",
					JSON.stringify({
						lat: Number.parseFloat(formatted[0]),
						lng: Number.parseFloat(formatted[1]),
					}),
				);
			}
		},
				...
				>
					...
					<ImageUploader
						...
						onImageUploaded={(imageResult) => {
							const array = [...(form.getValues("images") || [])];
					...
					<DocumentUploader
						...
						onDocumentUploaded={(documentResult) => {
							const array = [...(form.getValues("documents") || [])];
