...
export default function ProjectChoiceGuideResults() {
	...
	useEffect(() => {
		const loadedChoiceGuideResults = (data || []) as { createdAt: string }[];
	...
	const selectClick = (value: any) => {
		...
		const filteredData =
			ID === "0"
				? data
				: data?.filter(
						(choiceguideResult: any) =>
							choiceguideResult.widgetId.toString() === ID,
					);
				...
				action={
					<div className="flex flex-row w-full md:w-auto my-auto gap-4">
						...
						<Button
							...
							type="submit"
							onClick={() =>
								exportChoiceGuideToCSV(filterData, activeWidget, selectedWidget)
							}
