...
export default function ProjectResourceArguments() {
	...
	useEffect(() => {
		const allComments = (data || []) as {
			commentsFor?: Comment[];
			commentsAgainst?: Comment[];
		}[];

		const comments = allComments
			.flatMap(({ commentsFor = [], commentsAgainst = [] }) => [
				...commentsFor,
				...(commentsFor.flatMap(
					(comment: Comment) => comment.replies || [],
				) as Comment[]),
				...commentsAgainst,
				...(commentsAgainst.flatMap(
					(comment: Comment) => comment.replies || [],
				) as Comment[]),
			])
			.filter(
				(comment: Comment) =>
					comment.resourceId === Number.parseInt(id as string, 10),
			)
		...
		<div>
			<div className="container px-0">
				<div className="p-6 bg-white rounded-md">
					...
					>
						<ListHeading className="hidden lg:flex">Reactie ID</ListHeading>
					...
					<ul>
						{comments?.map((comment: any) => (
							<Link
								href={`/projects/${project}/comments/${comment.id}`}
								key={comment.id}
							>
								<li
									key={comment.id}
