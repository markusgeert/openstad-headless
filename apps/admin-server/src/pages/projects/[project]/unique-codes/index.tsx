...
export default function ProjectCodes() {
	...
	const { data: uniquecodes, resetUniqueCode } = useUniqueCodes(
		project as string,
	);
					...
					<div className="p-6 bg-white rounded-md clear-right">
						<div className="grid grid-cols-1 lg:grid-cols-4 items-center py-2 px-2 border-b border-border">
							...
							<ListHeading className="flex truncate">
								<button
									className="filter-button"
									onClick={(e) =>
										setFilterData(sortTable("code", e, filterData))
									}
								>
									Code
								</button>
						...
						<ul>
							{filterData?.map((code: any) => (
								<li
									key={code.id}
									className="grid grid-cols-2 lg:grid-cols-4 items-center py-3 px-2 hover:bg-muted hover:cursor-pointer transition-all duration-200 border-b"
								>
