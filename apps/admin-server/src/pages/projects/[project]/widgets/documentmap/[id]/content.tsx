				...
				>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel>
									Als de gebruiker een reactie kan toevoegen, wat is de tekst
									die boven het veld staat?
								</FormLabel>
								<FormControl>
									<Input {...field} />
								</FormControl>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel>
									Welke tekst wil je tonen wanneer het niet meer mogelijk is om
									te reageren?
								</FormLabel>
								<FormControl>
									<Input {...field} />
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel>
									Wat is de tekst bij het reactie overzicht wanneer er nog geen
									reacties zijn?
								</FormLabel>
								<FormControl>
									<Input {...field} />
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel>
									Wat is de tekst bij het reactie overzicht wanneer je niet bent
									ingelogd?
								</FormLabel>
								<FormControl>
									<Input {...field} />
					...
					<Heading size="lg" className="mt-8 mb-2">
						Marker Gerelateerd
					</Heading>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel>
									De gebruiker kan de markers aan- of uitzetten. Wat is de tekst
									van de knop?
								</FormLabel>
								<FormControl>
									<Input {...field} />
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel>Aan welke kant wil je de map tonen?</FormLabel>
								<Select
									onValueChange={field.onChange}
									value={field.value || "left"}
								>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel>
									Wil je dat de info pop-up standaard open staat wanneer de
									widget wordt ingeladen?
								</FormLabel>
								<Select
									onValueChange={field.onChange}
									value={field.value || "no"}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormDescription>
									Mist er een gedeelte van de URL? Voeg het hier toe
									(Bijvoorbeeld: /projecten). Dit wordt gebruikt bij het
									doorverwijzen naar de inzending. Dit is vooral belangrijk als
									de homepagina een &quot;/&quot; bevat, zoals bijvoorbeeld
									https://openstad.nl/projecten
								</FormDescription>
								<FormControl>
									<Input {...field} />
								</FormControl>
