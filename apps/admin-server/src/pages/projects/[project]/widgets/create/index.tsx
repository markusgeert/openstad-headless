...
export default function CreateWidget({}: WithApiUrlProps) {
	...
	async function submit(type: any, description: any) {
		...
		try {
			...
			if (widget) {
				toast.success(
					"Widget aangemaakt! Je wordt nu doorgestuurd naar de widget.",
				);
				setTimeout(() => {
					router.push(
						`/projects/${projectId}/widgets/${widget.type}/${widget.id}`,
					);
				}, 1500);
			}
		} catch (error) {
						...
						{filteredWidgets.map((type, key) => (
							<Card key={key}>
								<CardHeader>
									...
									<h2 className="text-lg font-regular">{type[1].name}</h2>
									...
									<Dialog>
										...
										<DialogContent>
											<DialogHeader>
												...
												<DialogDescription>
													<p className="mb-4">
														Geef de widget hier een logische naam, zodat deze
														makkelijk terug te vinden is.
													</p>
													<div
														className="grid w-full items-center gap-2 mt-4"
														style={{
															background: "#f4f4f4",
