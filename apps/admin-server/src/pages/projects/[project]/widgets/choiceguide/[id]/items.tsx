...
export default function WidgetChoiceGuideItems(
	...
	async function onSubmit(values: FormData) {
		if (selectedItem) {
			...
			setItems((currentItems) => [
				...currentItems,
				{
					trigger: `${currentItems.length > 0 ? Number.parseInt(currentItems[currentItems.length - 1].trigger) + 1 : 1}`,
					title: values.title,
	...
	useEffect(() => {
		...
		const dimensions = chosenConfig === "plane" ? ["X", "Y"] : ["X"];
								...
								<div className="p-6 bg-white rounded-md flex flex-col justify-between col-span-2">
									...
									>
										<div className="p-6 bg-white rounded-md flex flex-col justify-between col-span-2">
											...
											<Separator className="my-4" />
											<div
												className={`w-full col-span-full grid-cols-${dimensions.length + (form.watch("type") === "a-b-slider" ? 2 : 1)} grid gap-2 gap-y-2`}
											>
												<Heading size="lg">Vraaggroep titel</Heading>
												{dimensions.length > 0 &&
													dimensions.map((XY, i) => (
														<Heading key={i} size="lg">
															Weging {XY}
														</Heading>
													))}
											...
											<div className="w-full mt-4 flex flex-col gap-y-4">
												{["checkbox", "radiobox", "select"].includes(
													form.watch("type") || "",
												) ? (
													<>
														{widget?.config?.choiceOption?.choiceOptions?.map(
															(singleGroup: ChoiceOptions, index: number) => (
																<div
																	key={index}
																	className="w-full col-span-full grid-cols-1 grid"
																>
																	<Heading size="lg" className="mt-3">
																		{singleGroup.title}
																	</Heading>
																	<div
																		className={`w-full col-span-full grid-cols-${dimensions.length + (form.watch("type") === "a-b-slider" ? 2 : 1)} grid gap-2 gap-y-2 items-center`}
																		key={index}
																	>
																		{options.length > 0 &&
																			...
																			<>
																				<FormField
																					...
																					render={({ field }) => {
																						...
																						const watchValue = form.watch(
																							`weights.${singleGroup.id}.weight${XY}`,
																						);
																				...
																				<FormField
																					...
																					render={({ field }) => {
																						...
																						const watchValue = form.watch(
																							`weights.${singleGroup.id}.weightAB`,
																						);
